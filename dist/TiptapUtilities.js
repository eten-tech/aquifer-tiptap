...
OrderedMap.prototype = {
  ...
  // no such key exists.
  get: function(key) {
    var found4 = this.find(key);
    return found4 == -1 ? void 0 : this.content[found4 + 1];
  },
  ...
  update: function(key, value, newKey) {
    ...
    var found4 = self.find(key), content = self.content.slice();
    if (found4 == -1) {
      content.push(newKey || key, value);
    } else {
      content[found4 + 1] = value;
      if (newKey)
        content[found4] = newKey;
    }
  ...
  addBefore: function(place, key, value) {
    ...
    var found4 = without.find(place);
    content.splice(found4 == -1 ? content.length : found4, 0, key, value);
    return new OrderedMap(content);
  },
    ...
    function search(match, types) {
      ...
      for (let i = 0; i < match.next.length; i++) {
        ...
        if (!(type.isText || type.hasRequiredAttrs()) && seen.indexOf(next) == -1) {
          seen.push(next);
          let found4 = search(next, types.concat(type));
          if (found4)
            return found4;
        }
      }
...
function nullFrom(nfa4, node) {
  let result = [];
  scan(node);
  return result.sort(cmp);
  function scan(node2) {
    let edges = nfa4[node2];
...
function dfa(nfa4) {
  ...
  return explore(nullFrom(nfa4, 0));
  function explore(states) {
    let out = [];
    states.forEach((node) => {
      nfa4[node].forEach(({ term, to }) => {
        if (!term)
          return;
        let set;
        for (let i = 0; i < out.length; i++)
          if (out[i][0] == term)
            set = out[i][1];
        nullFrom(nfa4, to).forEach((node2) => {
          if (!set)
            out.push([term, set = []]);
    ...
    let state = labeled[states.join(",")] = new ContentMatch(states.indexOf(nfa4.length - 1) > -1);
    for (let i = 0; i < out.length; i++) {
  ...
  */
  static findFrom($pos, dir, textOnly = false) {
    ...
    for (let depth = $pos.depth - 1; depth >= 0; depth--) {
      let found4 = dir < 0 ? findSelectionIn($pos.node(0), $pos.node(depth), $pos.before(depth + 1), $pos.index(depth), dir, textOnly) : findSelectionIn($pos.node(0), $pos.node(depth), $pos.after(depth + 1), $pos.index(depth) + 1, dir, textOnly);
      if (found4)
        return found4;
    }
    return null;
  ...
  */
  static between($anchor, $head, bias) {
    ...
    if (!bias || dPos)
      bias = dPos >= 0 ? 1 : -1;
    if (!$head.parent.inlineContent) {
      let found4 = Selection.findFrom($head, bias, true) || Selection.findFrom($head, -bias, true);
      if (found4)
        $head = found4.$head;
      else
...
// node_modules/@tiptap/pm/node_modules/prosemirror-model/dist/index.js
...
var Slice2 = class _Slice {
  ...
  */
  removeBetween(from, to) {
    return new _Slice(removeRange2(this.content, from + this.openStart, to + this.openStart), this.openStart, this.openEnd);
  }
  /**
  Tests whether this slice is equal to another slice.
  */
...
function removeRange2(content, from, to) {
  let { index, offset } = content.findIndex(from), child = content.maybeChild(index);
  let { index: indexTo, offset: offsetTo } = content.findIndex(to);
  if (offset == from || child.isText) {
    if (offsetTo != to && !content.child(indexTo).isText)
  ...
  if (index != indexTo)
    throw new RangeError("Removing non-flat range");
  return content.replaceChild(index, child.copy(removeRange2(child.content, from - offset - 1, to - offset - 1)));
}
function insertInto2(content, dist, insert, parent) {
  ...
  let inner = insertInto2(child.content, dist - offset - 1, insert);
  return inner && content.replaceChild(index, child.copy(inner));
}
function replace2($from, $to, slice) {
  if (slice.openStart > $from.depth)
    throw new ReplaceError2("Inserted content deeper than insertion position");
  if ($from.depth - slice.openStart != $to.depth - slice.openEnd)
    throw new ReplaceError2("Inconsistent open depths");
  return replaceOuter2($from, $to, slice, 0);
}
function replaceOuter2($from, $to, slice, depth) {
  let index = $from.index(depth), node = $from.node(depth);
  if (index == $to.index(depth) && depth < $from.depth - slice.openStart) {
    let inner = replaceOuter2($from, $to, slice, depth + 1);
    return node.copy(node.content.replaceChild(index, inner));
  } else if (!slice.content.size) {
    ...
    let { start, end } = prepareSliceForReplace2(slice, $from);
...
function prepareSliceForReplace2(slice, $along) {
  let extra = $along.depth - slice.openStart, parent = $along.node(extra);
  let node = parent.copy(slice.content);
  for (let i = extra - 1; i >= 0; i--)
...
var ResolvedPos2 = class _ResolvedPos {
  ...
  */
  static resolveCached(doc2, pos) {
    let cache2 = resolveCache2.get(doc2);
    if (cache2) {
      ...
      resolveCache2.set(doc2, cache2 = new ResolveCache());
    }
    let result = cache2.elts[cache2.i] = _ResolvedPos.resolve(doc2, pos);
    cache2.i = (cache2.i + 1) % resolveCacheSize2;
    return result;
  }
};
...
var Node2 = class _Node {
  ...
  */
  replace(from, to, slice) {
    return replace2(this.resolve(from), this.resolve(to), slice);
  }
  /**
  Find the node directly after the given position.
  */
...
function dfa2(nfa4) {
  let labeled = /* @__PURE__ */ Object.create(null);
  return explore(nullFrom2(nfa4, 0));
  function explore(states) {
    let out = [];
    states.forEach((node) => {
      nfa4[node].forEach(({ term, to }) => {
        ...
        for (let i = 0; i < out.length; i++)
          if (out[i][0] == term)
            set = out[i][1];
        nullFrom2(nfa4, to).forEach((node2) => {
          if (!set)
            out.push([term, set = []]);
          if (set.indexOf(node2) == -1)
            set.push(node2);
    ...
    let state = labeled[states.join(",")] = new ContentMatch2(states.indexOf(nfa4.length - 1) > -1);
    for (let i = 0; i < out.length; i++) {
...
var NodeType = class _NodeType {
  ...
  */
  get isLeaf() {
    return this.contentMatch == ContentMatch2.empty;
  }
  /**
  ...
  */
  create(attrs = null, content, marks2) {
    ...
    return new Node2(this, this.computeAttrs(attrs), Fragment2.from(content), Mark2.setFrom(marks2));
  }
  /**
  ...
  */
  createChecked(attrs = null, content, marks2) {
    ...
    return new Node2(this, this.computeAttrs(attrs), content, Mark2.setFrom(marks2));
  }
  /**
  ...
  */
  createAndFill(attrs = null, content, marks2) {
    ...
    if (!after)
      return null;
    return new Node2(this, attrs, content.append(after), Mark2.setFrom(marks2));
  }
  /**
  ...
  */
  allowedMarks(marks2) {
    ...
    return !copy2 ? marks2 : copy2.length ? copy2 : Mark2.none;
  }
  /**
  @internal
  */
  static compile(nodes2, schema) {
    let result = /* @__PURE__ */ Object.create(null);
    nodes2.forEach((name, spec) => result[name] = new _NodeType(name, schema, spec));
...
var MarkType2 = class _MarkType {
  ...
  */
  constructor(name, rank, schema, spec) {
    ...
    this.instance = defaults ? new Mark2(this, defaults) : null;
  }
  /**
  ...
  */
  create(attrs = null) {
    ...
    return new Mark2(this, computeAttrs(this.attrs, attrs));
  }
  /**
  @internal
  */
...
var Schema = class {
  ...
  */
  constructor(spec) {
    ...
    this.marks = MarkType2.compile(this.spec.marks, this);
    let contentExprCache = /* @__PURE__ */ Object.create(null);
    for (let prop in this.nodes) {
      if (prop in this.marks)
        throw new RangeError(prop + " can not be both a node and a mark");
      let type = this.nodes[prop], contentExpr = type.spec.content || "", markExpr = type.spec.marks;
      type.contentMatch = contentExprCache[contentExpr] || (contentExprCache[contentExpr] = ContentMatch2.parse(contentExpr, this.nodes));
  ...
  */
  text(text, marks2) {
    ...
    return new TextNode(type, type.defaultAttrs, text, Mark2.setFrom(marks2));
  }
  /**
  ...
  */
  markFromJSON(json) {
    return Mark2.fromJSON(this, json);
  }
  /**
  @internal
  */
  nodeType(name) {
    let found4 = this.nodes[name];
    if (!found4)
...
var DOMParser = class _DOMParser {
  ...
  */
  parse(dom, options = {}) {
    ...
    context.addAll(dom, Mark2.none, options.from, options.to);
...
var ParseContext = class {
  constructor(parser, options, isOpen) {
    ...
    if (topNode)
      topContext = new NodeContext(topNode.type, topNode.attrs, Mark2.none, true, options.topMatch || topNode.type.contentMatch, topOptions);
    else if (isOpen)
      topContext = new NodeContext(null, null, Mark2.none, true, null, topOptions);
    else
      topContext = new NodeContext(parser.schema.topNodeType, null, Mark2.none, true, null, topOptions);
  ...
  addTextNode(dom, marks2) {
    ...
    if (top.options & OPT_PRESERVE_WS_FULL || top.inlineContext(dom) || /[^ \t\r\n\u000c]/.test(value)) {
      ...
      if (value)
        this.insertNode(this.parser.schema.text(value), marks2);
      this.findInText(dom);
    } else {
      this.findInside(dom);
    }
  ...
  ignoreFallback(dom, marks2) {
    if (dom.nodeName == "BR" && (!this.top.type || !this.top.type.inlineContent))
      this.findPlace(this.parser.schema.text("-"), marks2);
  }
  ...
  findPlace(node, marks2) {
    ...
    for (let depth = this.open; depth >= 0; depth--) {
      ...
      let found4 = cx.findWrapping(node);
      if (found4 && (!route || route.length > found4.length)) {
        route = found4;
        sync = cx;
        if (!found4.length)
          break;
...
// node_modules/@tiptap/pm/node_modules/prosemirror-transform/dist/index.js
var lower162 = 65535;
...
var AddMarkStep2 = class _AddMarkStep extends Step2 {
  ...
  */
  ...
  apply(doc2) {
    let oldSlice = doc2.slice(this.from, this.to), $from = doc2.resolve(this.from);
    let parent = $from.node($from.sharedDepth(this.to));
    let slice = new Slice2(mapFragment2(oldSlice.content, (node, parent2) => {
      if (!node.isAtom || !parent2.type.allowsMarkType(this.mark.type))
        return node;
      return node.mark(this.mark.addToSet(node.marks));
    }, parent), oldSlice.openStart, oldSlice.openEnd);
    return StepResult2.fromReplace(doc2, this.from, this.to, slice);
  }
  invert() {
...
var RemoveMarkStep2 = class _RemoveMarkStep extends Step2 {
  ...
  */
  ...
  apply(doc2) {
    let oldSlice = doc2.slice(this.from, this.to);
    let slice = new Slice2(mapFragment2(oldSlice.content, (node) => {
      return node.mark(this.mark.removeFromSet(node.marks));
    }, doc2), oldSlice.openStart, oldSlice.openEnd);
    return StepResult2.fromReplace(doc2, this.from, this.to, slice);
  }
...
var AddNodeMarkStep2 = class _AddNodeMarkStep extends Step2 {
  ...
  */
  ...
  apply(doc2) {
    ...
    let updated = node.type.create(node.attrs, null, this.mark.addToSet(node.marks));
    return StepResult2.fromReplace(doc2, this.pos, this.pos + 1, new Slice2(Fragment2.from(updated), 0, node.isLeaf ? 0 : 1));
  }
  invert(doc2) {
    let node = doc2.nodeAt(this.pos);
    if (node) {
...
var ReplaceStep2 = class _ReplaceStep extends Step2 {
  ...
  */
  ...
  apply(doc2) {
    ...
    return StepResult2.fromReplace(doc2, this.from, this.to, this.slice);
  }
  getMap() {
...
var ReplaceAroundStep2 = class _ReplaceAroundStep extends Step2 {
  ...
  */
  ...
  apply(doc2) {
    ...
    return StepResult2.fromReplace(doc2, this.from, this.to, inserted);
  }
  getMap() {
    return new StepMap2([
      this.from,
...
function contentBetween2(doc2, from, to) {
  let $from = doc2.resolve(from), dist = to - from, depth = $from.depth;
  while (dist > 0 && depth > 0 && $from.indexAfter(depth) == $from.node(depth).childCount) {
    depth--;
    dist--;
      ...
      for (let i = 0; i < marks2.length; i++) {
        if (!marks2[i].isInSet(newSet)) {
          if (removing && removing.to == start && removing.mark.eq(marks2[i]))
            ...
            removed.push(removing = new RemoveMarkStep2(start, end, marks2[i]));
        }
      }
      if (adding && adding.to == start)
        adding.to = end;
      else
        added.push(adding = new AddMarkStep2(start, end, mark));
    }
  });
...
function removeMark(tr, from, to, mark) {
  ...
  tr.doc.nodesBetween(from, to, (node, pos) => {
    ...
    if (mark instanceof MarkType2) {
      let set = node.marks, found4;
      while (found4 = mark.isInSet(set)) {
        (toRemove || (toRemove = [])).push(found4);
        set = found4.removeFromSet(set);
      }
    } else if (mark) {
  ...
  matched.forEach((m) => tr.step(new RemoveMarkStep2(m.from, m.to, m.style)));
}
function clearIncompatible(tr, pos, parentType, match = parentType.contentMatch, clearNewlines = true) {
  ...
  for (let i = 0; i < node.childCount; i++) {
    ...
    if (!allowed) {
      replSteps.push(new ReplaceStep2(cur, end, Slice2.empty));
    } else {
      match = allowed;
      for (let j = 0; j < child.marks.length; j++)
        if (!parentType.allowsMarkType(child.marks[j].type))
          tr.step(new RemoveMarkStep2(cur, end, child.marks[j]));
      if (clearNewlines && child.isText && parentType.whitespace != "pre") {
        let m, newline = /\r?\n|\r/g, slice;
        while (m = newline.exec(child.text)) {
          if (!slice)
            slice = new Slice2(Fragment2.from(parentType.schema.text(" ", parentType.allowedMarks(child.marks))), 0, 0);
          replSteps.push(new ReplaceStep2(cur + m.index, cur + m.index + m[0].length, slice));
        }
      }
    }
    cur = end;
...
function lift(tr, range, target) {
  ...
  tr.step(new ReplaceAroundStep2(start, end, gapStart, gapEnd, new Slice2(before.append(after), openStart, openEnd), before.size - openStart, true));
}
...
function wrap(tr, range, wrappers) {
  ...
  tr.step(new ReplaceAroundStep2(start, end, start, end, new Slice2(content, 0, 0), wrappers.length, true));
}
function setBlockType(tr, from, to, type, attrs) {
  ...
  tr.doc.nodesBetween(from, to, (node, pos) => {
    ...
    if (node.isTextblock && !node.hasMarkup(type, attrsHere) && canChangeType(tr.doc, tr.mapping.slice(mapFrom).map(pos), type)) {
      ...
      tr.step(new ReplaceAroundStep2(startM, endM, startM + 1, endM - 1, new Slice2(Fragment2.from(type.create(attrsHere, null, node.marks)), 0, 0), 1, true));
...
function setNodeMarkup(tr, pos, type, attrs, marks2) {
  ...
  tr.step(new ReplaceAroundStep2(pos, pos + node.nodeSize, pos + 1, pos + node.nodeSize - 1, new Slice2(Fragment2.from(newNode), 0, 0), 1, true));
}
function canSplit2(doc2, pos, depth = 1, typesAfter) {
  let $pos = doc2.resolve(pos), base2 = $pos.depth - depth;
  let innerType = typesAfter && typesAfter[typesAfter.length - 1] || $pos.parent;
  if (base2 < 0 || $pos.parent.type.spec.isolating || !$pos.parent.canReplace($pos.index(), $pos.parent.childCount) || !innerType.type.validContent($pos.parent.content.cutByIndex($pos.index(), $pos.parent.childCount)))
    return false;
...
function split(tr, pos, depth = 1, typesAfter) {
  ...
  tr.step(new ReplaceStep2(pos, pos, new Slice2(before.append(after), depth, depth), true));
}
function canJoin2(doc2, pos) {
  let $pos = doc2.resolve(pos), index = $pos.index();
  return joinable4($pos.nodeBefore, $pos.nodeAfter) && $pos.parent.canReplace(index, index + 1);
}
...
function joinPoint(doc2, pos, dir = -1) {
  ...
  for (let d = $pos.depth; ; d--) {
    ...
    if (before && !before.isTextblock && joinable4(before, after) && $pos.node(d).canReplace(index, index + 1))
      return pos;
    if (d == 0)
      break;
...
function join(tr, pos, depth) {
  ...
  tr.step(new ReplaceStep2(start, mapping.map(pos + depth, -1), Slice2.empty, true));
  if (convertNewlines === true) {
...
function replaceStep(doc2, from, to = from, slice = Slice2.empty) {
  if (from == to && !slice.size)
    return null;
  let $from = doc2.resolve(from), $to = doc2.resolve(to);
  if (fitsTrivially($from, $to, slice))
    return new ReplaceStep2(from, to, slice);
...
var Fitter = class {
  ...
  fit() {
    ...
    let slice = new Slice2(content, openStart, openEnd);
    if (moveInline > -1)
      return new ReplaceAroundStep2($from.pos, moveInline, this.$to.pos, this.$to.end(), slice, placedSize);
    if (slice.size || $from.pos != this.$to.pos)
      return new ReplaceStep2($from.pos, $to.pos, slice);
    return null;
  }
  ...
  close($to) {
    let close4 = this.findCloseLevel($to);
    if (!close4)
      return null;
    while (this.depth > close4.depth)
      this.closeFrontierNode();
    if (close4.fit.childCount)
      this.placed = addToFragment(this.placed, close4.depth, close4.fit);
    $to = close4.move;
    for (let d = close4.depth + 1; d <= $to.depth; d++) {
...
function replaceRange(tr, from, to, slice) {
  ...
  if (fitsTrivially($from, $to, slice))
    return tr.step(new ReplaceStep2(from, to, slice));
...
function replaceRangeWith(tr, from, to, node) {
  ...
  tr.replaceRange(from, to, new Slice2(Fragment2.from(node), 0, 0));
}
...
var Transform2 = class {
  ...
  */
  step(step) {
    ...
    if (result.failed)
      throw new TransformError3(result.failed);
    return this;
  }
  /**
  ...
  */
  replaceWith(from, to, content) {
    return this.replace(from, to, new Slice2(Fragment2.from(content), 0, 0));
  }
  /**
  ...
  */
  setNodeAttribute(pos, attr, value) {
    this.step(new AttrStep2(pos, attr, value));
    return this;
  }
  /**
  ...
  */
  setDocAttribute(attr, value) {
    this.step(new DocAttrStep2(attr, value));
    return this;
  }
  /**
  ...
  */
  addNodeMark(pos, mark) {
    this.step(new AddNodeMarkStep2(pos, mark));
    return this;
  }
  /**
  ...
  */
  removeNodeMark(pos, mark) {
    ...
    this.step(new RemoveNodeMarkStep2(pos, mark));
    return this;
  }
  /**
...
// node_modules/prosemirror-commands/node_modules/prosemirror-transform/node_modules/prosemirror-model/dist/index.js
function findDiffStart3(a, b, pos) {
  for (let i = 0; ; i++) {
    if (i == a.childCount || i == b.childCount)
      return a.childCount == b.childCount ? null : pos;
    let childA = a.child(i), childB = b.child(i);
    ...
    if (childA.content.size || childB.content.size) {
      let inner = findDiffStart3(childA.content, childB.content, pos + 1);
      if (inner != null)
        return inner;
    }
    pos += childA.nodeSize;
...
function findDiffEnd3(a, b, posA, posB) {
  for (let iA = a.childCount, iB = b.childCount; ; ) {
    if (iA == 0 || iB == 0)
      return iA == iB ? null : { a: posA, b: posB };
    let childA = a.child(--iA), childB = b.child(--iB), size = childA.nodeSize;
    ...
    if (childA.content.size || childB.content.size) {
      let inner = findDiffEnd3(childA.content, childB.content, posA - 1, posB - 1);
      if (inner)
        return inner;
    }
    posA -= size;
...
var Fragment3 = class _Fragment {
  ...
  */
  findDiffStart(other, pos = 0) {
    return findDiffStart3(this, other, pos);
  }
  /**
  Find the first position, searching from the end, at which this
  fragment and the given fragment differ, or `null` if they are
  ...
  */
  findDiffEnd(other, pos = this.size, otherPos = other.size) {
    return findDiffEnd3(this, other, pos, otherPos);
  }
  /**
  Find the index and inner offset corresponding to a given relative
  position in this fragment. The result object will be reused
  ...
  */
  findIndex(pos, round = -1) {
    if (pos == 0)
      return retIndex3(0, pos);
    if (pos == this.size)
      return retIndex3(this.content.length, pos);
    if (pos > this.size || pos < 0)
      throw new RangeError(`Position ${pos} outside of fragment (${this})`);
    for (let i = 0, curPos = 0; ; i++) {
      let cur = this.child(i), end = curPos + cur.nodeSize;
      if (end >= pos) {
        if (end == pos || round > 0)
          return retIndex3(i + 1, end);
        return retIndex3(i, curPos);
      }
      curPos = end;
    }
  }
...
var found3 = { index: 0, offset: 0 };
function retIndex3(index, offset) {
  found3.index = index;
  found3.offset = offset;
  return found3;
}
function compareDeep3(a, b) {
  if (a === b)
    return true;
  if (!(a && typeof a == "object") || !(b && typeof b == "object"))
    return false;
  ...
  if (array) {
    if (a.length != b.length)
      return false;
    for (let i = 0; i < a.length; i++)
      if (!compareDeep3(a[i], b[i]))
        return false;
  } else {
    for (let p in a)
      if (!(p in b) || !compareDeep3(a[p], b[p]))
        return false;
    for (let p in b)
      if (!(p in a))
        return false;
...
var Mark3 = class _Mark {
  ...
  */
  eq(other) {
    return this == other || this.type == other.type && compareDeep3(this.attrs, other.attrs);
  }
  /**
  Convert this mark to a JSON-serializeable representation.
  */
...
Mark3.none = [];
var ReplaceError3 = class extends Error {
};
var Slice3 = class _Slice {
  /**
  Create a slice. When specifying a non-zero open depth, you must
  make sure that there are nodes of at least that depth at the
  appropriate side of the fragment—i.e. if the fragment is an
  ...
  */
  insertAt(pos, fragment) {
    let content = insertInto3(this.content, pos + this.openStart, fragment);
    return content && new _Slice(content, this.openStart, this.openEnd);
  }
  /**
  @internal
  */
  removeBetween(from, to) {
    return new _Slice(removeRange3(this.content, from + this.openStart, to + this.openStart), this.openStart, this.openEnd);
  }
  /**
  Tests whether this slice is equal to another slice.
  */
...
function removeRange3(content, from, to) {
  let { index, offset } = content.findIndex(from), child = content.maybeChild(index);
  let { index: indexTo, offset: offsetTo } = content.findIndex(to);
  if (offset == from || child.isText) {
    if (offsetTo != to && !content.child(indexTo).isText)
  ...
  if (index != indexTo)
    throw new RangeError("Removing non-flat range");
  return content.replaceChild(index, child.copy(removeRange3(child.content, from - offset - 1, to - offset - 1)));
}
function insertInto3(content, dist, insert, parent) {
  let { index, offset } = content.findIndex(dist), child = content.maybeChild(index);
  if (offset == dist || child.isText) {
    if (parent && !parent.canReplace(index, index, insert))
      return null;
  ...
  let inner = insertInto3(child.content, dist - offset - 1, insert);
  return inner && content.replaceChild(index, child.copy(inner));
}
function replace3($from, $to, slice) {
  if (slice.openStart > $from.depth)
    throw new ReplaceError3("Inserted content deeper than insertion position");
  if ($from.depth - slice.openStart != $to.depth - slice.openEnd)
    throw new ReplaceError3("Inconsistent open depths");
  return replaceOuter3($from, $to, slice, 0);
}
function replaceOuter3($from, $to, slice, depth) {
  let index = $from.index(depth), node = $from.node(depth);
  if (index == $to.index(depth) && depth < $from.depth - slice.openStart) {
    let inner = replaceOuter3($from, $to, slice, depth + 1);
    return node.copy(node.content.replaceChild(index, inner));
  } else if (!slice.content.size) {
    return close3(node, replaceTwoWay3($from, $to, depth));
  } else if (!slice.openStart && !slice.openEnd && $from.depth == depth && $to.depth == depth) {
    let parent = $from.parent, content = parent.content;
    return close3(parent, content.cut(0, $from.parentOffset).append(slice.content).append(content.cut($to.parentOffset)));
  } else {
    let { start, end } = prepareSliceForReplace3(slice, $from);
    return close3(node, replaceThreeWay3($from, start, end, $to, depth));
  }
}
function checkJoin3(main, sub) {
  if (!sub.type.compatibleContent(main.type))
    throw new ReplaceError3("Cannot join " + sub.type.name + " onto " + main.type.name);
}
function joinable5($before, $after, depth) {
  let node = $before.node(depth);
  checkJoin3(node, $after.node(depth));
  return node;
}
function addNode3(child, target) {
  let last = target.length - 1;
  if (last >= 0 && child.isText && child.sameMarkup(target[last]))
    target[last] = child.withText(target[last].text + child.text);
  else
...
function addRange3($start, $end, depth, target) {
  ...
  for (let i = startIndex; i < endIndex; i++)
    addNode3(node.child(i), target);
  if ($end && $end.depth == depth && $end.textOffset)
    addNode3($end.nodeBefore, target);
}
function close3(node, content) {
  node.type.checkContent(content);
  return node.copy(content);
}
function replaceThreeWay3($from, $start, $end, $to, depth) {
  let openStart = $from.depth > depth && joinable5($from, $start, depth + 1);
  let openEnd = $to.depth > depth && joinable5($end, $to, depth + 1);
  let content = [];
  addRange3(null, $from, depth, content);
  if (openStart && openEnd && $start.index(depth) == $end.index(depth)) {
    checkJoin3(openStart, openEnd);
    addNode3(close3(openStart, replaceThreeWay3($from, $start, $end, $to, depth + 1)), content);
  } else {
    if (openStart)
      addNode3(close3(openStart, replaceTwoWay3($from, $start, depth + 1)), content);
    addRange3($start, $end, depth, content);
    if (openEnd)
      addNode3(close3(openEnd, replaceTwoWay3($end, $to, depth + 1)), content);
  }
  addRange3($to, null, depth, content);
  return new Fragment3(content);
}
function replaceTwoWay3($from, $to, depth) {
  let content = [];
  addRange3(null, $from, depth, content);
  if ($from.depth > depth) {
    let type = joinable5($from, $to, depth + 1);
    addNode3(close3(type, replaceTwoWay3($from, $to, depth + 1)), content);
  }
  addRange3($to, null, depth, content);
  return new Fragment3(content);
}
function prepareSliceForReplace3(slice, $along) {
  let extra = $along.depth - slice.openStart, parent = $along.node(extra);
  let node = parent.copy(slice.content);
  for (let i = extra - 1; i >= 0; i--)
...
var ResolvedPos3 = class _ResolvedPos {
  /**
  @internal
  */
  constructor(pos, path, parentOffset) {
  ...
  */
  static resolveCached(doc2, pos) {
    let cache2 = resolveCache3.get(doc2);
    if (cache2) {
      for (let i = 0; i < cache2.elts.length; i++) {
        let elt = cache2.elts[i];
        if (elt.pos == pos)
      ...
      resolveCache3.set(doc2, cache2 = new ResolveCache2());
    }
    let result = cache2.elts[cache2.i] = _ResolvedPos.resolve(doc2, pos);
    cache2.i = (cache2.i + 1) % resolveCacheSize3;
    return result;
  }
};
...
var resolveCacheSize3 = 12;
var resolveCache3 = /* @__PURE__ */ new WeakMap();
var NodeRange3 = class {
  /**
  Construct a node range. `$from` and `$to` should point into the
  same node until at least the given `depth`, since a node range
  denotes an adjacent set of nodes in a single parent node.
...
var emptyAttrs3 = /* @__PURE__ */ Object.create(null);
var Node3 = class _Node {
  /**
  @internal
  */
  constructor(type, attrs, content, marks2 = Mark3.none) {
    this.type = type;
    this.attrs = attrs;
    this.marks = marks2;
  ...
  */
  hasMarkup(type, attrs, marks2) {
    return this.type == type && compareDeep3(this.attrs, attrs || type.defaultAttrs || emptyAttrs3) && Mark3.sameSet(this.marks, marks2 || Mark3.none);
  }
  /**
  Create a new node with the same markup as this node, containing
  the given content (or empty, if no content is given).
  ...
  */
  slice(from, to = this.content.size, includeParents = false) {
    ...
    let start = $from.start(depth), node = $from.node(depth);
    let content = node.content.cut($from.pos - start, $to.pos - start);
    return new Slice3(content, $from.depth - depth, $to.depth - depth);
  }
  /**
  Replace the part of the document between the given positions with
  the given slice. The slice must 'fit', meaning its open sides
  ...
  */
  replace(from, to, slice) {
    return replace3(this.resolve(from), this.resolve(to), slice);
  }
  /**
  Find the node directly after the given position.
  */
  ...
  */
  toString() {
    ...
    if (this.content.size)
      name += "(" + this.content.toStringInner() + ")";
    return wrapMarks3(this.marks, name);
  }
  /**
  Get the content match in this node at the given index.
  */
...
function wrapMarks3(marks2, str) {
  for (let i = marks2.length - 1; i >= 0; i--)
    str = marks2[i].type.name + "(" + str + ")";
  return str;
}
var ContentMatch3 = class _ContentMatch {
  /**
  @internal
  */
  constructor(validEnd) {
  ...
  */
  static parse(string, nodeTypes) {
    let stream = new TokenStream3(string, nodeTypes);
    if (stream.next == null)
      return _ContentMatch.empty;
    let expr = parseExpr3(stream);
    if (stream.next)
      stream.err("Unexpected trailing text");
    let match = dfa3(nfa3(expr));
    checkForDeadEnds3(match, stream);
    return match;
  }
  /**
  Match a node type, returning a match after that node if
...
function parseExpr3(stream) {
  let exprs = [];
  do {
    exprs.push(parseExprSeq3(stream));
  } while (stream.eat("|"));
  return exprs.length == 1 ? exprs[0] : { type: "choice", exprs };
}
function parseExprSeq3(stream) {
  let exprs = [];
  do {
    exprs.push(parseExprSubscript3(stream));
  } while (stream.next && stream.next != ")" && stream.next != "|");
  return exprs.length == 1 ? exprs[0] : { type: "seq", exprs };
}
function parseExprSubscript3(stream) {
  let expr = parseExprAtom3(stream);
  for (; ; ) {
    if (stream.eat("+"))
      expr = { type: "plus", expr };
    else if (stream.eat("*"))
      ...
      expr = parseExprRange3(stream, expr);
    else
      break;
  }
  return expr;
}
function parseNum3(stream) {
  if (/\D/.test(stream.next))
    stream.err("Expected number, got '" + stream.next + "'");
  let result = Number(stream.next);
  stream.pos++;
...
function parseExprRange3(stream, expr) {
  let min = parseNum3(stream), max = min;
  if (stream.eat(",")) {
    if (stream.next != "}")
      max = parseNum3(stream);
    else
      max = -1;
  }
  if (!stream.eat("}"))
...
function resolveName3(stream, name) {
  let types = stream.nodeTypes, type = types[name];
  if (type)
    return [type];
  let result = [];
...
function parseExprAtom3(stream) {
  if (stream.eat("(")) {
    let expr = parseExpr3(stream);
    if (!stream.eat(")"))
      stream.err("Missing closing paren");
    ...
    let exprs = resolveName3(stream, stream.next).map((type) => {
      if (stream.inline == null)
        stream.inline = type.isInline;
      else if (stream.inline != type.isInline)
        stream.err("Mixing inline and block content");
...
function nullFrom3(nfa4, node) {
  let result = [];
  scan(node);
  return result.sort(cmp3);
  function scan(node2) {
    let edges = nfa4[node2];
    if (edges.length == 1 && !edges[0].term)
      return scan(edges[0].to);
...
function dfa3(nfa4) {
  let labeled = /* @__PURE__ */ Object.create(null);
  return explore(nullFrom3(nfa4, 0));
  function explore(states) {
    let out = [];
    states.forEach((node) => {
      nfa4[node].forEach(({ term, to }) => {
        ...
        for (let i = 0; i < out.length; i++)
          if (out[i][0] == term)
            set = out[i][1];
        nullFrom3(nfa4, to).forEach((node2) => {
          if (!set)
            out.push([term, set = []]);
          if (set.indexOf(node2) == -1)
            set.push(node2);
    ...
    let state = labeled[states.join(",")] = new ContentMatch3(states.indexOf(nfa4.length - 1) > -1);
    for (let i = 0; i < out.length; i++) {
      let states2 = out[i][1].sort(cmp3);
      state.next.push({ type: out[i][0], next: labeled[states2.join(",")] || explore(states2) });
    }
    return state;
  }
}
function checkForDeadEnds3(match, stream) {
  for (let i = 0, work = [match]; i < work.length; i++) {
    let state = work[i], dead = !state.validEnd, nodes2 = [];
    for (let j = 0; j < state.next.length; j++) {
      let { type, next } = state.next[j];
...
// node_modules/prosemirror-commands/node_modules/prosemirror-transform/dist/index.js
var lower163 = 65535;
...
var AddMarkStep3 = class _AddMarkStep extends Step3 {
  ...
  */
  ...
  apply(doc2) {
    let oldSlice = doc2.slice(this.from, this.to), $from = doc2.resolve(this.from);
    let parent = $from.node($from.sharedDepth(this.to));
    let slice = new Slice3(mapFragment3(oldSlice.content, (node, parent2) => {
      if (!node.isAtom || !parent2.type.allowsMarkType(this.mark.type))
        return node;
      return node.mark(this.mark.addToSet(node.marks));
    }, parent), oldSlice.openStart, oldSlice.openEnd);
    return StepResult3.fromReplace(doc2, this.from, this.to, slice);
  }
  invert() {
...
var RemoveMarkStep3 = class _RemoveMarkStep extends Step3 {
  ...
  */
  ...
  apply(doc2) {
    let oldSlice = doc2.slice(this.from, this.to);
    let slice = new Slice3(mapFragment3(oldSlice.content, (node) => {
      return node.mark(this.mark.removeFromSet(node.marks));
    }, doc2), oldSlice.openStart, oldSlice.openEnd);
    return StepResult3.fromReplace(doc2, this.from, this.to, slice);
  }
...
var AddNodeMarkStep3 = class _AddNodeMarkStep extends Step3 {
  ...
  */
  ...
  apply(doc2) {
    ...
    let updated = node.type.create(node.attrs, null, this.mark.addToSet(node.marks));
    return StepResult3.fromReplace(doc2, this.pos, this.pos + 1, new Slice3(Fragment3.from(updated), 0, node.isLeaf ? 0 : 1));
  }
  invert(doc2) {
    let node = doc2.nodeAt(this.pos);
    if (node) {
...
var ReplaceStep3 = class _ReplaceStep extends Step3 {
  /**
  The given `slice` should fit the 'gap' between `from` and
  `to`—the depths must line up, and the surrounding nodes must be
  able to be joined with the open sides of the slice. When
  ...
  */
  ...
  apply(doc2) {
    if (this.structure && contentBetween3(doc2, this.from, this.to))
  ...
  merge(other) {
    if (!(other instanceof _ReplaceStep) || other.structure || this.structure)
      return null;
    if (this.from + this.slice.size == other.from && !this.slice.openEnd && !other.slice.openStart) {
      let slice = this.slice.size + other.slice.size == 0 ? Slice3.empty : new Slice3(this.slice.content.append(other.slice.content), this.slice.openStart, other.slice.openEnd);
      return new _ReplaceStep(this.from, this.to + (other.to - other.from), slice, this.structure);
    } else if (other.to == this.from && !this.slice.openStart && !other.slice.openEnd) {
      let slice = this.slice.size + other.slice.size == 0 ? Slice3.empty : new Slice3(other.slice.content.append(this.slice.content), other.slice.openStart, this.slice.openEnd);
      return new _ReplaceStep(other.from, this.to, slice, this.structure);
    } else {
      return null;
    }
  ...
  */
  static fromJSON(schema, json) {
    if (typeof json.from != "number" || typeof json.to != "number")
      throw new RangeError("Invalid input for ReplaceStep.fromJSON");
    return new _ReplaceStep(json.from, json.to, Slice3.fromJSON(schema, json.slice), !!json.structure);
  }
};
Step3.jsonID("replace", ReplaceStep3);
var ReplaceAroundStep3 = class _ReplaceAroundStep extends Step3 {
  /**
  Create a replace-around step with the given range and gap.
  `insert` should be the point in the slice into which the content
  of the gap should be moved. `structure` has the same meaning as
  ...
  */
  ...
  apply(doc2) {
    if (this.structure && (contentBetween3(doc2, this.from, this.gapFrom) || contentBetween3(doc2, this.gapTo, this.to)))
  ...
  */
  static fromJSON(schema, json) {
    if (typeof json.from != "number" || typeof json.to != "number" || typeof json.gapFrom != "number" || typeof json.gapTo != "number" || typeof json.insert != "number")
      throw new RangeError("Invalid input for ReplaceAroundStep.fromJSON");
    return new _ReplaceAroundStep(json.from, json.to, json.gapFrom, json.gapTo, Slice3.fromJSON(schema, json.slice), json.insert, !!json.structure);
  }
};
Step3.jsonID("replaceAround", ReplaceAroundStep3);
function contentBetween3(doc2, from, to) {
  let $from = doc2.resolve(from), dist = to - from, depth = $from.depth;
  while (dist > 0 && depth > 0 && $from.indexAfter(depth) == $from.node(depth).childCount) {
    depth--;
    dist--;
...
function canCut3(node, start, end) {
  return (start == 0 || node.canReplace(start, node.childCount)) && (end == node.childCount || node.canReplace(0, end));
}
function liftTarget3(range) {
  let parent = range.parent;
  let content = parent.content.cutByIndex(range.startIndex, range.endIndex);
  for (let depth = range.depth; ; --depth) {
    let node = range.$from.node(depth);
    ...
    if (depth < range.depth && node.canReplace(index, endIndex, content))
      return depth;
    if (depth == 0 || node.type.spec.isolating || !canCut3(node, index, endIndex))
      break;
  }
  return null;
}
function findWrapping3(range, nodeType, attrs = null, innerRange = range) {
  let around = findWrappingOutside3(range, nodeType);
  let inner = around && findWrappingInside3(innerRange, nodeType);
  if (!inner)
    return null;
  return around.map(withAttrs3).concat({ type: nodeType, attrs }).concat(inner.map(withAttrs3));
}
function withAttrs3(type) {
  return { type, attrs: null };
}
function findWrappingOutside3(range, type) {
  let { parent, startIndex, endIndex } = range;
  let around = parent.contentMatchAt(startIndex).findWrapping(type);
  if (!around)
    return null;
...
function findWrappingInside3(range, type) {
  let { parent, startIndex, endIndex } = range;
  let inner = parent.child(startIndex);
  let inside = type.contentMatch.findWrapping(inner.type);
  if (!inside)
...
function canSplit3(doc2, pos, depth = 1, typesAfter) {
  let $pos = doc2.resolve(pos), base2 = $pos.depth - depth;
  let innerType = typesAfter && typesAfter[typesAfter.length - 1] || $pos.parent;
  if (base2 < 0 || $pos.parent.type.spec.isolating || !$pos.parent.canReplace($pos.index(), $pos.parent.childCount) || !innerType.type.validContent($pos.parent.content.cutByIndex($pos.index(), $pos.parent.childCount)))
    return false;
...
function canJoin3(doc2, pos) {
  let $pos = doc2.resolve(pos), index = $pos.index();
  return joinable6($pos.nodeBefore, $pos.nodeAfter) && $pos.parent.canReplace(index, index + 1);
}
function canAppendWithSubstitutedLinebreaks2(a, b) {
  if (!b.content.size)
    a.type.compatibleContent(b.type);
  let match = a.contentMatchAt(a.childCount);
  let { linebreakReplacement } = a.type.schema;
...
function joinable6(a, b) {
  return !!(a && b && !a.isLeaf && canAppendWithSubstitutedLinebreaks2(a, b));
}
function joinPoint2(doc2, pos, dir = -1) {
  let $pos = doc2.resolve(pos);
  for (let d = $pos.depth; ; d--) {
    let before, after, index = $pos.index(d);
    if (d == $pos.depth) {
    ...
    if (before && !before.isTextblock && joinable6(before, after) && $pos.node(d).canReplace(index, index + 1))
      return pos;
    if (d == 0)
      break;
    pos = dir < 0 ? $pos.before(d) : $pos.after(d);
...
function replaceStep2(doc2, from, to = from, slice = Slice3.empty) {
  if (from == to && !slice.size)
    return null;
  let $from = doc2.resolve(from), $to = doc2.resolve(to);
  if (fitsTrivially2($from, $to, slice))
    return new ReplaceStep3(from, to, slice);
  return new Fitter2($from, $to, slice).fit();
}
function fitsTrivially2($from, $to, slice) {
  return !slice.openStart && !slice.openEnd && $from.start() == $to.start() && $from.parent.canReplace($from.index(), $to.index(), slice.content);
}
var Fitter2 = class {
  constructor($from, $to, unplaced) {
  ...
  fit() {
    ...
    let slice = new Slice3(content, openStart, openEnd);
    if (moveInline > -1)
      return new ReplaceAroundStep3($from.pos, moveInline, this.$to.pos, this.$to.end(), slice, placedSize);
    if (slice.size || $from.pos != this.$to.pos)
      return new ReplaceStep3($from.pos, $to.pos, slice);
    return null;
  }
  // Find a position on the start spine of `this.unplaced` that has
  // content that can be moved somewhere on the frontier. Returns two
  ...
  openMore() {
    let { content, openStart, openEnd } = this.unplaced;
    let inner = contentAt2(content, openStart);
    if (!inner.childCount || inner.firstChild.isLeaf)
      return false;
    this.unplaced = new Slice3(content, openStart + 1, Math.max(openEnd, inner.size + openStart >= content.size - openEnd ? openStart + 1 : 0));
    return true;
  }
  dropNode() {
    let { content, openStart, openEnd } = this.unplaced;
    ...
    if (inner.childCount <= 1 && openStart > 0) {
      let openAtEnd = content.size - openStart <= openStart + inner.size;
      this.unplaced = new Slice3(dropFromFragment2(content, openStart - 1, 1), openStart - 1, openAtEnd ? openStart - 1 : openEnd);
    } else {
      this.unplaced = new Slice3(dropFromFragment2(content, openStart, 1), openStart, openEnd);
    }
  }
  // Move content from the unplaced slice at `sliceDepth` to the
  // frontier node at `frontierDepth`. Close that frontier node when
  ...
  placeNodes({ sliceDepth, frontierDepth, parent, inject, wrap: wrap2 }) {
    ...
    while (taken < fragment.childCount) {
      ...
      if (taken > 1 || openStart == 0 || next.content.size) {
        match = matches2;
        add.push(closeNodeStart2(next.mark(type.allowedMarks(next.marks)), taken == 1 ? openStart : 0, taken == fragment.childCount ? openEndCount : -1));
      }
    }
    let toEnd = taken == fragment.childCount;
    if (!toEnd)
      openEndCount = -1;
    this.placed = addToFragment2(this.placed, frontierDepth, Fragment3.from(add));
    this.frontier[frontierDepth].match = match;
    if (toEnd && openEndCount < 0 && parent && parent.type == this.frontier[this.depth].type && this.frontier.length > 1)
      this.closeFrontierNode();
    for (let i = 0, cur = fragment; i < openEndCount; i++) {
    ...
    this.unplaced = !toEnd ? new Slice3(dropFromFragment2(slice.content, sliceDepth, taken), slice.openStart, slice.openEnd) : sliceDepth == 0 ? Slice3.empty : new Slice3(dropFromFragment2(slice.content, sliceDepth - 1, 1), sliceDepth - 1, openEndCount < 0 ? slice.openEnd : sliceDepth - 1);
  }
  mustMoveInline() {
    if (!this.$to.parent.isTextblock)
      return -1;
    let top = this.frontier[this.depth], level;
    if (!top.type.isTextblock || !contentAfterFits2(this.$to, this.$to.depth, top.type, top.match, false) || this.$to.depth == this.depth && (level = this.findCloseLevel(this.$to)) && level.depth == this.depth)
      return -1;
    let { depth } = this.$to, after = this.$to.after(depth);
    while (depth > 1 && after == this.$to.end(--depth))
      ++after;
  ...
  findCloseLevel($to) {
    scan:
      for (let i = Math.min(this.depth, $to.depth); i >= 0; i--) {
        let { match, type } = this.frontier[i];
        let dropInner = i < $to.depth && $to.end(i + 1) == $to.pos + ($to.depth - (i + 1));
        let fit = contentAfterFits2($to, i, type, match, dropInner);
        if (!fit)
          continue;
        for (let d = i - 1; d >= 0; d--) {
          let { match: match2, type: type2 } = this.frontier[d];
          let matches2 = contentAfterFits2($to, d, type2, match2, true);
          if (!matches2 || matches2.childCount)
            continue scan;
        }
        return { depth: i, fit, move: dropInner ? $to.doc.resolve($to.after(i + 1)) : $to };
  ...
  close($to) {
    ...
    while (this.depth > close4.depth)
      this.closeFrontierNode();
    if (close4.fit.childCount)
      this.placed = addToFragment2(this.placed, close4.depth, close4.fit);
    $to = close4.move;
    for (let d = close4.depth + 1; d <= $to.depth; d++) {
      let node = $to.node(d), add = node.type.contentMatch.fillBefore(node.content, true, $to.index(d));
      this.openFrontierNode(node.type, node.attrs, add);
  ...
  openFrontierNode(type, attrs = null, content) {
    let top = this.frontier[this.depth];
    top.match = top.match.matchType(type);
    this.placed = addToFragment2(this.placed, this.depth, Fragment3.from(type.create(attrs, content)));
    this.frontier.push({ type, match: type.contentMatch });
  }
  closeFrontierNode() {
    let open = this.frontier.pop();
    ...
    if (add.childCount)
      this.placed = addToFragment2(this.placed, this.frontier.length, add);
  }
};
...
function addToFragment2(fragment, depth, content) {
  if (depth == 0)
    return fragment.append(content);
  return fragment.replaceChild(fragment.childCount - 1, fragment.lastChild.copy(addToFragment2(fragment.lastChild.content, depth - 1, content)));
}
...
function closeNodeStart2(node, openStart, openEnd) {
  if (openStart <= 0)
    return node;
  let frag = node.content;
  if (openStart > 1)
...
function contentAfterFits2($to, depth, type, match, open) {
  let node = $to.node(depth), index = open ? $to.indexAfter(depth) : $to.index(depth);
  if (index == node.childCount && !type.compatibleContent(node.type))
    return null;
  let fit = match.fillBefore(node.content, true, index);
  return fit && !invalidMarks2(type, node.content, index) ? fit : null;
}
function invalidMarks2(type, fragment, start) {
  for (let i = start; i < fragment.childCount; i++)
    if (!type.allowsMarks(fragment.child(i).marks))
      return true;
  return false;
...
var joinBackward = (state, dispatch, view) => {
  ...
  if (!$cut) {
    let range = $cursor.blockRange(), target = range && liftTarget3(range);
    if (target == null)
      return false;
    if (dispatch)
  ...
  if ($cursor.parent.content.size == 0 && (textblockAt(before, "end") || NodeSelection.isSelectable(before))) {
    for (let depth = $cursor.depth; ; depth--) {
      let delStep = replaceStep2(state.doc, $cursor.before(depth), $cursor.after(depth), Slice.empty);
...
function joinTextblocksAround(state, $cut, dispatch) {
  ...
  let step = replaceStep2(state.doc, beforePos, afterPos, Slice.empty);
  if (!step || step.from != beforePos || step instanceof ReplaceStep3 && step.slice.size >= afterPos - beforePos)
    return false;
  if (dispatch) {
...
var joinForward = (state, dispatch, view) => {
  ...
  if ($cursor.parent.content.size == 0 && (textblockAt(after, "start") || NodeSelection.isSelectable(after))) {
    let delStep = replaceStep2(state.doc, $cursor.before(), $cursor.after(), Slice.empty);
    if (delStep && delStep.slice.size < delStep.to - delStep.from) {
      if (dispatch) {
        let tr = state.tr.step(delStep);
...
var joinUp = (state, dispatch) => {
  ...
  if (nodeSel) {
    if (sel.node.isTextblock || !canJoin3(state.doc, sel.from))
      return false;
    point = sel.from;
  } else {
    point = joinPoint2(state.doc, sel.from, -1);
    if (point == null)
      return false;
  }
  if (dispatch) {
...
var joinDown = (state, dispatch) => {
  ...
  if (sel instanceof NodeSelection) {
    if (sel.node.isTextblock || !canJoin3(state.doc, sel.to))
      return false;
    point = sel.to;
  } else {
    point = joinPoint2(state.doc, sel.to, 1);
    if (point == null)
      return false;
  }
  if (dispatch)
...
var lift2 = (state, dispatch) => {
  ...
  let range = $from.blockRange($to), target = range && liftTarget3(range);
  if (target == null)
    return false;
  if (dispatch)
    dispatch(state.tr.lift(range, target).scrollIntoView());
...
function splitBlockAs(splitNode) {
  return (state, dispatch) => {
    ...
    if (state.selection instanceof NodeSelection && state.selection.node.isBlock) {
      if (!$from.parentOffset || !canSplit3(state.doc, $from.pos))
        return false;
      if (dispatch)
    ...
    let can = canSplit3(tr.doc, tr.mapping.map($from.pos), 1, types);
    if (!types && !can && canSplit3(tr.doc, tr.mapping.map($from.pos), 1, deflt ? [{ type: deflt }] : void 0)) {
      if (deflt)
        types = [{ type: deflt }];
      can = true;
    }
...
function joinMaybeClear(state, $pos, dispatch) {
  ...
  if (!$pos.parent.canReplace(index, index + 1) || !(after.isTextblock || canJoin3(state.doc, $pos.pos)))
    return false;
  if (dispatch)
...
function deleteBarrier(state, $cut, dispatch, dir) {
  ...
  if (canDelAfter && (conn = (match = before.contentMatchAt(before.childCount)).findWrapping(after.type)) && match.matchType(conn[0] || after.type).validEnd) {
    if (dispatch) {
      ...
      let tr = state.tr.step(new ReplaceAroundStep3($cut.pos - 1, end, $cut.pos, end, new Slice(wrap2, 1, 0), conn.length, true));
      let $joinAt = tr.doc.resolve(end + 2 * conn.length);
      if ($joinAt.nodeAfter && $joinAt.nodeAfter.type == before.type && canJoin3(tr.doc, $joinAt.pos))
        tr.join($joinAt.pos);
      dispatch(tr.scrollIntoView());
    }
    return true;
  ...
  let range = selAfter && selAfter.$from.blockRange(selAfter.$to), target = range && liftTarget3(range);
  ...
  if (canDelAfter && textblockAt(after, "start", true) && textblockAt(before, "end")) {
    ...
    if (at.canReplace(at.childCount, at.childCount, afterText.content)) {
      if (dispatch) {
        ...
        let tr = state.tr.step(new ReplaceAroundStep3($cut.pos - wrap2.length, $cut.pos + after.nodeSize, $cut.pos + afterDepth, $cut.pos + after.nodeSize - afterDepth, new Slice(end, wrap2.length, 0), 0, true));
        dispatch(tr.scrollIntoView());
      }
      return true;
    }
...
function wrapIn(nodeType, attrs = null) {
  return function(state, dispatch) {
    let { $from, $to } = state.selection;
    let range = $from.blockRange($to), wrapping = range && findWrapping3(range, nodeType, attrs);
    if (!wrapping)
      return false;
    if (dispatch)
...
function getSchemaByResolvedExtensions(extensions, editor) {
  ...
  const nodes2 = Object.fromEntries(nodeExtensions.map((extension) => {
  ...
  get plugins() {
    ...
    const allPlugins = extensions.map((extension) => {
      ...
      if (extension.type === "mark" && getExtensionField(extension, "exitable", context)) {
        defaultBindings.ArrowRight = () => Mark4.handleExit({ editor, mark: extension });
      }
...
var setContent = (content, emitUpdate = false, parseOptions = {}, options = {}) => ({ editor, tr, dispatch, commands: commands2 }) => {
  ...
  if (parseOptions.preserveWhitespace !== "full") {
    ...
    if (dispatch) {
      tr.replaceWith(0, doc2.content.size, document3).setMeta("preventUpdate", !emitUpdate);
    }
    return true;
  }
  if (dispatch) {
...
function combineTransactionSteps(oldDoc, transactions) {
  const transform = new Transform2(oldDoc);
...
var splitBlock2 = ({ keepMarks = true } = {}) => ({ tr, state, dispatch, editor }) => {
  ...
  if (selection instanceof NodeSelection && selection.node.isBlock) {
    if (!$from.parentOffset || !canSplit2(doc2, $from.pos)) {
      return false;
    }
    if (dispatch) {
      if (keepMarks) {
...
var splitListItem = (typeOrName, overrideAttrs = {}) => ({ tr, state, dispatch, editor }) => {
  ...
  if ($from.parent.content.size === 0 && $from.node(-1).childCount === $from.indexAfter(-1)) {
    ...
    if (dispatch) {
      ...
      tr.replace(start, $from.after(-depthAfter), new Slice2(wrap2, 4 - depthBefore, 0));
      let sel = -1;
...
// node_modules/zeed-dom/dist/chunk-YMRUYF4I.js
      ...
      for (const part of rules2) {
        ...
        if (type === "attribute") {
          if (action === "equals") {
            ...
            success = !!((_a2 = element2.getAttribute(name)) == null ? void 0 : _a2.startsWith(value));
            if (debug)
...
var _VNode = class _VNode2 {
  ...
  appendChild(node) {
    ...
    } else if (node instanceof _VNode2) {
      node.remove();
  ...
  remove() {
    var _a2;
    (_a2 = this == null ? void 0 : this.parentNode) == null ? void 0 : _a2.removeChild(this);
    return this;
  }
  /** Replace content of node with text or nodes */
  replaceChildren(...nodes2) {
    this._childNodes = nodes2.map(
  ...
  replaceWith(...nodes2) {
    ...
    if (p) {
      ...
      if (index >= 0) {
        ...
        p._childNodes.splice(index, 1, ...nodes2);
        this._parentNode = null;
  ...
  flattenNodes() {
    ...
    nodes2.push(this);
    for (const child of this._childNodes)
      nodes2.push(...child.flattenNodes());
    return nodes2;
  }
  render() {
    return "";
  ...
  get ownerDocument() {
    var _a2;
    if (this.nodeType === _VNode2.DOCUMENT_NODE || this.nodeType === _VNode2.DOCUMENT_FRAGMENT_NODE)
      return this;
    return (_a2 = this == null ? void 0 : this._parentNode) == null ? void 0 : _a2.ownerDocument;
  }
  toString() {
...
var VTextNode = class extends VNode {
  ...
  render() {
    var _a2;
    const parentTagName = (_a2 = this.parentNode) == null ? void 0 : _a2.tagName;
...
var VNodeQuery = class extends VNode {
  ...
  parent(selector) {
    ...
    if (this.parentNode == null)
      return null;
    return (_a2 = this.parentNode) == null ? void 0 : _a2.parent(selector);
  }
...
var VElement = class extends VNodeQuery {
  ...
  get classList() {
    var _a2;
    const classNames = String((_a2 = this.className) != null ? _a2 : "").trim().split(/\s+/g) || [];
    return {
      contains(s) {
...
var VHTMLDocument = class extends VDocument {
  ...
  get title() {
    var _a2;
    return ((_a2 = this.querySelector("title")) == null ? void 0 : _a2.textContent) || "";
  }
  set title(title) {
...
// node_modules/zeed-dom/dist/chunk-MR7V3FOC.js
...
var HtmlParser = class {
  ...
  parseAttributes(tagName, input) {
    ...
    input.replace(this.attrRe, (...m) => {
      ...
      attrs[name] = (_b = (_a2 = valueInSingleQuote != null ? valueInSingleQuote : valueInQuote) != null ? _a2 : value) != null ? _b : true;
      return void 0;
    });
    return attrs;
  }
  ...
  const parser = new HtmlParser({
    ...
    scanner: {
      ...
      characters(text) {
        ...
        const parentNode = stack[stack.length - 1];
        if (((_a2 = parentNode == null ? void 0 : parentNode.lastChild) == null ? void 0 : _a2.nodeType) === VNode.TEXT_NODE) {
...
function getHTMLFromFragment(doc2, schema, options) {
  const zeedDocument = DOMSerializer.fromSchema(schema).serializeFragment(doc2.content, {
...
function generateHTML(doc2, extensions) {
  ...
  const contentNode = Node2.fromJSON(schema, doc2);
  return getHTMLFromFragment(contentNode, schema);
}
function generateJSON(html2, extensions, options) {
  const schema = getSchema(extensions);
  ...
  return new Plugin({
    ...
    props: {
      handlePaste: (view, event, slice) => {
        ...
        const link = (0, import_linkifyjs.find)(textContent, { defaultProtocol: options.defaultProtocol }).find((item) => item.isLink && item.value === textContent);
        if (!textContent || !link) {
          return false;
        }
      ...
      markPasteRule({
        find: (text) => {
          ...
          if (text) {
            ...
            const links = (0, import_linkifyjs.find)(text).filter((item) => item.isLink && validate(item.value));
            if (links.length) {
...
// src/custom-extensions/video.ts
var video_default = Node4.create({
  name: "video",
  group: "block",
  addAttributes() {
    return {
...
var bible_reference_default = Mark4.create({
  name: "bibleReference",
  priority: 1001,
  keepOnSplit: false,
  addAttributes() {
  ...
  renderHTML({ HTMLAttributes }) {
    const verses = HTMLAttributes.verses.map(({ startVerse, endVerse }) => [
...
// src/custom-extensions/resource-reference.ts
var resource_reference_default = Mark4.create({
...
// src/custom-extensions/comments.ts
var comments_default = Mark4.create({
  name: "comments",
  priority: 1001,
  keepOnSplit: false,
  excludes: "",
...
// src/custom-extensions/open-translators-notes/general-comment.ts
var general_comment_default = Node4.create({
    ...
    return [
      {
        tag: "div",
        getAttrs: (element) => {
          if (element.getAttribute("data-bnType") === "OpenTranslatorsNotesGeneralComment") {
            return {};
          }
          return false;
        }
...
// src/custom-extensions/open-translators-notes/paragraph-note.ts
var paragraph_note_default = Node4.create({
...
// src/custom-extensions/open-translators-notes/section-note.ts
var section_note_default = Node4.create({
...
// src/custom-extensions/open-translators-notes/translation-options.ts
var translation_options_default = Node4.create({
...
// src/custom-extensions/open-translators-notes/translation-options/default-translation-option.ts
var default_translation_option_default = Node4.create({
...
// src/custom-extensions/open-translators-notes/translation-options/additional-translation-options.ts
var additional_translation_options_default = Node4.create({
...
// src/custom-extensions/implied.ts
var implied_default = Mark4.create({
  name: "implied",
  priority: 1001,
  keepOnSplit: false,
  parseHTML() {
...
// src/custom-extensions/index.ts
var ParagraphWithIndentation = extendNodeWithIndentOption(Paragraph);
var BulletListWithIndentation = extendNodeWithIndentOption(BulletList);
var OrderedListWithIndentation = extendNodeWithIndentOption(OrderedList);
var BlockquoteWithIndentation = extendNodeWithIndentOption(Blockquote);
...
var officialExtensions = [CharacterCount];
var customExtensions = [
  ...
  translation_options_default,
  default_translation_option_default,
  additional_translation_options_default,
...
function parseHtmlAsJson(html2) {
  try {
    return JSON.stringify(generateJSON(html2, allExtensions));
  } catch (error) {
    console.error(error);
    return "";
  }
